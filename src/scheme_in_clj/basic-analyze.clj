...
(defn analyze-sequence [exp]
  (defn sequentially [proc1 proc2]
    (fn [env]
      (proc1 env)
      (proc2 env)))
  (defn loop-inner [first-proc rest-procs]
    (if (nil? rest-procs)
      first-proc
      (loop-inner (sequentially first-proc (car rest-procs))
                  (cdr rest-procs))))
  (let [procs (map analyze exp)]
    (if (nil? procs)
      (throw (Exception. (str "Empty sequence -- ANALYZE" exp)))
      (loop-inner (car procs) (cdr procs)))))
